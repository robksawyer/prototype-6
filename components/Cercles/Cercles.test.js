...
describe('components', () => {
  describe('Cercles', () => {
    it('should mount with props', function () {
      const wrap = mount(<Cercles>Hello World</Cercles>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<Cercles>Hello World</Cercles>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('cercles')
